...
public class Corpus {
	 ...
	 * @throws IOException
	 */
	public static int[][] getEntities(File[] files) throws IOException {

		int[][] entities = new int[files.length][];
		...
		for (File file : files) {
			...
			if (lines.length != 1)
				entities[doc_no] = new int[lines.length];
			else
				entities[doc_no] = new int[0];
			...
			for (int i = 0; i < entities[doc_no].length; i++) {
				...
				entities[doc_no][i] = Integer.parseInt(temp[0]);

				// System.out.println(entities[doc_no][i] + "\t" + doc_no);
