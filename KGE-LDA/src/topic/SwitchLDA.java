...
/**
 * Switch GibbsSampling
 * 
 * @author: Liang Yao
 * @email yaoliang@zju.edu.cn
 */

public class SwitchLDA {
	...
	public SwitchLDA(int[][] documents, int[][] entities, int V, int E) {
	...
	public void markovChain(int K, double alpha, double beta, double beta_bar, double gamma, int iterations) {
	...
	int sampleFullConditionalEntity(int d, int m) {
		...
		for (int k = 0; k < K; k++) {
			...
			p[k] = (nd[d][k] + alpha) / (ndsum[d] + K * alpha) * (ne[entities[d][m]][k] + beta_bar)
					/ (nesum[k] + E * beta_bar) * (entity_count[k] + gamma)
					/ (entity_count[k] + word_count[k] + 2 * gamma);
